...
# Open the CSV file welches durch ausführen von openai_query.py erzeugt wurde
with open('openai-llm-keywords.csv', mode='r', newline='') as file:
    ...
    for row in csv_reader:
        for element in row:
            # leere Einträge überspringen sonst an Liste anhängen
            if any(x.strip() for x in element):
                keywords_llm.append(ast.literal_eval(element))
...
# Habe ich am Ende nicht verwendet da durch die Verwendung von extrahierten Keywords
# zu wenige vorhanden waren, um das Ergebnis zu verbessern
...
# Visuelle Darstellung als .html file mit pyLDAvis für das Modell erstellen und Filename beginnend mit Anzahl der Topics speichern
    ...
    pyLDAvis.save_html(vis, str(num_top)+'_keybert_llm.html')
